...
join acs_1_housing_metro_politan_2021 B on A.metro_politan_area  = B.metro_politan_area
...
create or replace view housing_price_census_tract_fl_viz as ( select
    E.*,
    C._2016_housing_price,
    C._2021_housing_price,
    C.housing_price_change,
    C._2016_housing_units,
    C._2021_housing_units,
    C.housing_unit_change 
from
    income_census_tract_fl_viz E  
join
    (select
        A."name",
        A.state,
        A.county,
        A.tract,
        A._value__owner_occupied_units__median_dollars as _2016_housing_price,
        B._value__owner_occupied_units__median_dollars as _2021_housing_price,
        round(cast(((B._value__owner_occupied_units__median_dollars -  A._value__owner_occupied_units__median_dollars)/ A._value__owner_occupied_units__median_dollars)*100 as numeric),
        4) as housing_price_change,
    ...
    where
        ...
        and B._value__owner_occupied_units__median_dollars != 0) C 
        on E.name = C.name )


-- TX
 create or replace view housing_price_census_tract_tx_viz as ( select
    E.*,
    C._2016_housing_price,
    C._2021_housing_price,
    C.housing_price_change,
...
select * from housing_price_census_tract_tx_viz

--CA
--create or replace view housing_price_census_tract_ca_viz as (
--select E.*, C._2016_housing_price, C._2021_housing_price, C.housing_price_change, C._2016_housing_units, C._2021_housing_units, C.housing_unit_change from income_census_tract_ca_viz E 
